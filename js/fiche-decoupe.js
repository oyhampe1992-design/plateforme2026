...
(function() {
  ...
  var TEMPLATE_EG_BOIS = 'templates/EG_BOIS.xlsx';
  var TEMPLATE_SM_BOIS = 'templates/SM_BOIS.xlsx';
  ...
  // ════════════════════════════════════════════════════════════════
  //  EXPORT EG BOIS / LABABOIS
  // ════════════════════════════════════════════════════════════════
  // Particularités EG BOIS :
  //  - Coche chants : "x" MINUSCULE (la formule de calcul auto en
  //    colonne I est sensible à la casse : IF(E17="x",...))
  //  - Pas de cellule épaisseur chant ni Devis/Commande
  //  - Colonne I contient des formules auto à NE PAS TOUCHER
  //  - 65 lignes data possibles (rows 17 à 81)
  function exportEgBois(piecesAll, infos) {
    if (typeof JSZip === 'undefined') { alert('JSZip non chargé.'); return; }
    ...
    fetch(TEMPLATE_EG_BOIS)
      .then(function(r) {
        if (!r.ok) throw new Error('Template introuvable : ' + TEMPLATE_EG_BOIS + ' (HTTP ' + r.status + ')');
        return r.arrayBuffer();
      })
      .then(function(buf) { return JSZip.loadAsync(buf); })
      .then(function(zip) {
        return zip.file('xl/worksheets/sheet1.xml').async('string').then(function(xml) {
          ...
          // En-tête chantier
          xml = setCellString(xml, 'C4',  infos.nom);          // NOM
          xml = setCellString(xml, 'I4',  infos.refChantier);  // Ref chantier
          xml = setCellString(xml, 'N4',  infos.societe);      // De: → société

          // En-tête matière
          xml = setCellString(xml, 'C10', infos.typeMatiere);  // Marque/couleur
          xml = setCellString(xml, 'G10', infos.epMatiere);    // Épaisseur

          // Sens du fil : cocher OUI par défaut (X en L10)
          xml = setCellString(xml, 'L10', 'X');

          // Pièces (lignes 17+, max 65 jusqu'à 81)
          // Colonnes : A (nom), B (longueur), C (largeur), D (qté)
          // Chants : E (long1), F (long2), G (larg1), H (larg2) — coche "x" minuscule
          // Colonne I : formule auto, NE PAS TOUCHER
          var MAX_ROWS = 65;
          var nbInsere = Math.min(pieces.length, MAX_ROWS);
          var stats = { etagereInversee: 0 };

          for (var i = 0; i < nbInsere; i++) {
            var row = 17 + i, pc = pieces[i], c = chantsCotes(pc);
            if (pc.type === 'etagere' && c.l1) stats.etagereInversee++;

            xml = setCellString(xml, 'A' + row, pc.designation || '');
            xml = setCellNumber(xml, 'B' + row, pc.longueur);
            xml = setCellNumber(xml, 'C' + row, pc.largeur);
            xml = setCellNumber(xml, 'D' + row, pc.nombre || 1);
            // ⚠ "x" minuscule pour que la formule de calcul auto fonctionne
            if (c.L1) xml = setCellString(xml, 'E' + row, 'x');
            if (c.L2) xml = setCellString(xml, 'F' + row, 'x');
            if (c.l1) xml = setCellString(xml, 'G' + row, 'x');
            if (c.l2) xml = setCellString(xml, 'H' + row, 'x');
          }
          ...
          return zip.generateAsync({
            ...
            var refSafe = (infos.refChantier || 'export').replace(/[^A-Za-z0-9_-]/g, '_').substring(0, 30);
            var epTag = epCible != null ? ('_' + epCible + 'mm') : '';
            var filename = 'EG_BOIS_' + refSafe + epTag + '_' + stampNow() + '.xlsx';
            telechargerBlob(blob, filename);
            console.log('[fiche-decoupe v11] Export EG BOIS : ' + filename
                      + ' (' + nbInsere + ' pièces, ' + stats.etagereInversee + ' étagère(s) inversée(s))');
            if (alerts.length) alert('⚠ ' + alerts.join('\n\n'));
          });
        });
      ...
      .catch(function(err) {
        alert('Erreur export EG BOIS :\n' + err.message);
        console.error('[fiche-decoupe]', err);
      });
  }

  // ════════════════════════════════════════════════════════════════
  //  EXPORT SM BOIS (Claye-Souilly 77)
  // ════════════════════════════════════════════════════════════════
  // Particularités SM BOIS :
  //  - Une seule feuille "FORMULAIRE DECOUPE SM BOIS"
  //  - Coche chants : "O" pour OUI, "N" pour NON (4 colonnes : A1/A2/B1/B2)
  //  - 24 lignes data max (rows 16 à 39)
  //  - Nom/Prénom dans 2 cellules séparées (C4 nom, C5 prénom)
  //  - Pas de Devis/Commande
  //  - Colonne A = formules d'auto-numérotation, NE PAS TOUCHER
  function exportSmBois(piecesAll, infos) {
    if (typeof JSZip === 'undefined') { alert('JSZip non chargé.'); return; }
    ...
    // Split Nom/Prénom (convention SM BOIS : C4=Nom famille, C5=Prénom)
    // "Pierre Oyhampe" → prénom="Pierre", nom="Oyhampe"
    var nomFamille = (infos.nom || '').trim(), prenom = '';
    var parts = nomFamille.split(/\s+/);
    if (parts.length >= 2) {
      prenom = parts[0];
      nomFamille = parts.slice(1).join(' ');
    }

    fetch(TEMPLATE_SM_BOIS)
      .then(function(r) {
        if (!r.ok) throw new Error('Template introuvable : ' + TEMPLATE_SM_BOIS + ' (HTTP ' + r.status + ')');
        return r.arrayBuffer();
      })
      .then(function(buf) { return JSZip.loadAsync(buf); })
      .then(function(zip) {
        return zip.file('xl/worksheets/sheet1.xml').async('string').then(function(xml) {
          ...
          // En-tête chantier (colonne C)
          xml = setCellString(xml, 'C3',  infos.societe);
          xml = setCellString(xml, 'C4',  nomFamille);
          xml = setCellString(xml, 'C5',  prenom);
          xml = setCellString(xml, 'C6',  infos.adresse);
          // C7 (CP) et C8 (Ville) laissés vides — pas dans le modal
          xml = setCellNumber(xml, 'C9',  infos.tel);
          xml = setCellString(xml, 'C10', infos.mail);

          // En-tête matière/chant/chantier (colonne K)
          xml = setCellString(xml, 'K3', infos.typeMatiere);
          var epMat = parseEp(infos.epMatiere);
          if (epMat != null) xml = setCellNumber(xml, 'K4', epMat);
          xml = setCellString(xml, 'K5', infos.typeChant);
          var epChant = parseEp(infos.epChant);
          if (epChant != null) xml = setCellNumber(xml, 'K6', epChant);
          xml = setCellString(xml, 'K7', infos.refChantier);

          // Pièces (lignes 16+, max 24 jusqu'à 39)
          // Colonnes : B (nom), G (longueur), H (largeur), I (qté)
          // Chants : J (A1=long1), K (A2=long2), L (B1=larg1), M (B2=larg2)
          // Valeurs : "O" si chant présent, "N" si absent
          // ⚠ Colonne A = formules auto-numérotation, NE PAS TOUCHER
          var MAX_ROWS = 24;
          var nbInsere = Math.min(pieces.length, MAX_ROWS);
          var stats = { etagereInversee: 0 };

          for (var i = 0; i < nbInsere; i++) {
            var row = 16 + i, pc = pieces[i], c = chantsCotes(pc);
            if (pc.type === 'etagere' && c.l1) stats.etagereInversee++;

            xml = setCellString(xml, 'B' + row, pc.designation || '');
            xml = setCellNumber(xml, 'G' + row, pc.longueur);
            xml = setCellNumber(xml, 'H' + row, pc.largeur);
            xml = setCellNumber(xml, 'I' + row, pc.nombre || 1);
            xml = setCellString(xml, 'J' + row, c.L1 ? 'O' : 'N');
            xml = setCellString(xml, 'K' + row, c.L2 ? 'O' : 'N');
            xml = setCellString(xml, 'L' + row, c.l1 ? 'O' : 'N');
            xml = setCellString(xml, 'M' + row, c.l2 ? 'O' : 'N');
          }
          ...
          return zip.generateAsync({
            ...
            var refSafe = (infos.refChantier || 'export').replace(/[^A-Za-z0-9_-]/g, '_').substring(0, 30);
            var epTag = epCible != null ? ('_' + epCible + 'mm') : '';
            var filename = 'SM_BOIS_' + refSafe + epTag + '_' + stampNow() + '.xlsx';
            telechargerBlob(blob, filename);
            console.log('[fiche-decoupe v12] Export SM BOIS : ' + filename
                      + ' (' + nbInsere + ' pièces, ' + stats.etagereInversee + ' étagère(s) inversée(s))');
            if (alerts.length) alert('⚠ ' + alerts.join('\n\n'));
          });
        });
  ...
  function ouvrirModal() {
    ...
    var estFourn = (fournisseur === 'dispano' || fournisseur === 'bouney' || fournisseur === 'eg_bois' || fournisseur === 'sm_bois');
    var afficheFourn = estFourn ? 'block' : 'none';
    var afficheDispano = (fournisseur === 'dispano') ? 'block' : 'none';
    var afficheEpChant = (fournisseur === 'dispano' || fournisseur === 'sm_bois') ? 'block' : 'none';
    var afficheSociete = estFourn ? 'block' : 'none';
    ...
    modal.innerHTML = ''
      ...
      +     '<option value="eg_bois"' + (fournisseur==='eg_bois' ?' selected':'') + '>EG BOIS / LABABOIS</option>'
      +     '<option value="sm_bois"' + (fournisseur==='sm_bois' ?' selected':'') + '>SM BOIS</option>'
      +   '</select>'
      ...
      +     '<div id="fd_zoneEpChant" style="display:' + afficheEpChant + '">'
    ...
    document.getElementById('fd_fournisseur').addEventListener('change', function(e) {
      ...
      var est = (v === 'dispano' || v === 'bouney' || v === 'eg_bois' || v === 'sm_bois');
      document.getElementById('fd_zoneFourn').style.display    = est ? 'block' : 'none';
      document.getElementById('fd_zoneSociete').style.display  = est ? 'block' : 'none';
      document.getElementById('fd_zoneEpChant').style.display  = (v === 'dispano' || v === 'sm_bois') ? 'block' : 'none';
    ...
    document.getElementById('fd_generer').addEventListener('click', function() {
      ...
      else if (fr === 'eg_bois') exportEgBois(window._cutlistPieces || [], infos);
      else if (fr === 'sm_bois') exportSmBois(window._cutlistPieces || [], infos);
    });
  }
  ...
  function injecterBouton() {
    ...
    bar.appendChild(btn);
    console.log('[fiche-decoupe v12] Bouton injecté (Dispano + Bouney + EG BOIS + SM BOIS)');
  }
