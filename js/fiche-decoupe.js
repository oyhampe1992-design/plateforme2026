/* ================================================================
   THE WOODER — fiche-decoupe.js (v6 — étagère inversion-aware)
   ================================================================
   Module ADDITIF — ne touche à aucun fichier existant.

   Changement v6 vs v5 :
   - Détection auto de l'inversion étagère via window._profEtag :
     après cutlist.js, pour une étagère, une des dimensions
     (longueur ou largeur) est REMPLACÉE par profEtag. L'autre est
     la largeur intérieure (le côté visible où va le chant).
     → Chant placé sur le bon côté Dispano (Av ou G) selon où se
       trouve le côté visible dans le débit final.
   - Idem côté format générique (chant L1 ou l1 selon inversion).

   Mapping Dispano final :
     - porte / tiroir              → Av + Ar + G + D
     - plinthe                     → Av + Ar + G (2 long + 1 larg)
     - lateral / montant / panneau → Av (front longueur)
     - étagère :
         * largeur intérieure côté longueur → Av (cas classique)
         * largeur intérieure côté largeur  → G  (cas inversé)
...
(function() {
  ...
  // ── Détection : pour une étagère, quel côté porte la largeur
  //    intérieure (côté visible avec chant) ?
  //    Retour : 'L' (côté longueur) ou 'l' (côté largeur)
  //    Par défaut 'L' si profEtag indéterminé ou ambigu.
  function coteLargeurInterieureEtagere(pc) {
    var profE = window._profEtag;
    if (profE == null) return 'L'; // défaut
    var TOL = 1; // tolérance 1mm
    var longueurEstProf = Math.abs(Number(pc.longueur) - profE) <= TOL;
    var largeurEstProf  = Math.abs(Number(pc.largeur)  - profE) <= TOL;
    if (longueurEstProf && !largeurEstProf) return 'l'; // inversion
    return 'L'; // cas classique
  }
  ...
  function exportDispano(piecesAll, infos) {
    ...
    if (epCible != null) {
      ...
      for (var i = 0; i < piecesAll.length; i++) {
        if (Number(piecesAll[i].epaisseur) === epCible) pieces.push(piecesAll[i]);
        else exclues.push(piecesAll[i]);
      }
    }
      ...
      .then(function(zip) {
        return zip.file('xl/worksheets/sheet1.xml').async('string').then(function(xml) {
          ...
          for (var i = 0; i < nbInsere; i++) {
            ...
            // Compter les inversions étagère pour log
            if (pc.type === 'etagere' && c.G) nbInversions++;
          ...
          if (exclues.length) {
            ...
            alerts.push(exclues.length + ' pièce(s) exclues : ' + listEp.join(', ')
                      + '\n→ relancer l\'export avec ces épaisseurs');
          }
          ...
          return zip.generateAsync({
            ...
            console.log('[fiche-decoupe v6] Export Dispano : ' + filename
                      + ' (' + nbInsere + ' pièces, ' + nbInversions + ' étagère(s) inversée(s))');
  ...
  function injecterBouton() {
    ...
    bar.appendChild(btn);
    console.log('[fiche-decoupe v6] Bouton injecté (étagère inversion-aware)');
  }
