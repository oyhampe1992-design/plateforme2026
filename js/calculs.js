...
// ════════════════════════════════════════════════════════════════
// CONFIG POSE — mode de support + plinthe (paramètres globaux projet)
// ════════════════════════════════════════════════════════════════
// Lit les 4 inputs DOM décrivant la pose du meuble et retourne un objet
// avec une "position dérivée" du panneau inférieur, qui est ce qui pilote
// les formules dans le reste de calculs.js.
//
// Inputs DOM lus :
//   - modeSupport       : 'lateraux_porteurs' | 'nivelement_independant' | 'suspendu'
//   - typePlinthe       : 'encastree' | 'applique' | 'aucune'   (legacy, conservé)
//   - moyenNivellement  : 'pieds' | 'structure'                  (utile pour devis/quincaillerie)
//   - plintheRapportee  : checkbox booléenne
//
// Sortie : objet config + champ dérivé .positionPanneauInf qui vaut
//   'au_dessus_plinthe'   → panneau inf posé sur la plinthe (yBas = lPl + ep/2)
//   'dans_zone_plinthe'   → panneau inf chevauche la plinthe (yBas = lPl - ep/2)
//   'fleur'               → panneau inf à fleur du bas des latéraux (yBas = ep/2)
//
// Rétrocompat : si modeSupport absent du DOM (ancien HTML), on tombe sur
// 'lateraux_porteurs' et la lecture de typePlinthe seul reproduit le
// comportement antérieur à l'identique.
function lireConfigPose() {
  var elMode = document.getElementById('modeSupport');
  var elTP   = document.getElementById('typePlinthe');
  var elMN   = document.getElementById('moyenNivellement');
  var elPR   = document.getElementById('plintheRapportee');

  var modeSupport      = (elMode && elMode.value) || 'lateraux_porteurs';
  var typePlinthe      = (elTP   && elTP.value)   || 'encastree';
  var moyenNivellement = (elMN   && elMN.value)   || 'pieds';
  var plintheRapportee = !!(elPR && elPR.checked);

  var positionPanneauInf;
  if (modeSupport === 'lateraux_porteurs') {
    if (typePlinthe === 'encastree')      positionPanneauInf = 'au_dessus_plinthe';
    else if (typePlinthe === 'applique')  positionPanneauInf = 'dans_zone_plinthe';
    else                                  positionPanneauInf = 'fleur';
  } else {
    // nivelement_independant ou suspendu : panneau inf toujours à fleur
    positionPanneauInf = 'fleur';
  }

  // hasPlinthe = "y a-t-il une pièce plinthe à prendre en compte côté caisson ?"
  // Important : en mode nivelement avec plinthe rapportée, la plinthe est
  // DEVANT le caisson (pleine façade), elle ne participe pas aux formules
  // de yBas/yPD du caisson lui-même. Donc côté géométrie caisson, hasPlinthe = false.
  var hasPlintheCaisson;
  if (modeSupport === 'lateraux_porteurs') {
    hasPlintheCaisson = (typePlinthe === 'encastree' || typePlinthe === 'applique');
  } else {
    hasPlintheCaisson = false;
  }

  return {
    modeSupport:        modeSupport,
    typePlinthe:        typePlinthe,
    moyenNivellement:   moyenNivellement,
    plintheRapportee:   plintheRapportee,
    positionPanneauInf: positionPanneauInf,
    hasPlintheCaisson:  hasPlintheCaisson
  };
}
...
// la longueur correspond à la hauteur intérieure du meuble selon la pose :
//   - position au-dessus plinthe (latéraux porteurs + encastrée) : hLat - ep_sup - ep_inf - hPlinthe
//   - position dans zone plinthe  (latéraux porteurs + applique)  : hLat - ep_sup - hPlinthe (fond plinthe libre)
//   - position à fleur            (sans plinthe sur latéraux)     : hLat - ep_sup - ep_inf
...
function classifierMontants(items) {
  ...
  // En mode nivelement/suspendu, la pièce plinthe (si présente) est rapportée
  // pleine façade DEVANT le caisson : elle ne participe pas à la hauteur intérieure.
  var hPlEffective = cfg.hasPlintheCaisson ? hPl : 0;
  // Hauteur attendue d'un montant plein (tolérance ±5mm pour arrondi PDF)
  var hPlein;
  if (cfg.positionPanneauInf === 'au_dessus_plinthe')      hPlein = hLat - ep0 - ep0 - hPlEffective;
  else if (cfg.positionPanneauInf === 'dans_zone_plinthe') hPlein = hLat - ep0 - hPlEffective;
  else                                                      hPlein = hLat - ep0 - ep0; // fleur
...
function calculerConnecteurs(items) {
  ...
  // En mode nivelement/suspendu, la plinthe (rapportée pleine façade) ne joue
  // pas sur le yBas du caisson : on neutralise lPl pour le calcul.
  var lPlEff = cfg.hasPlintheCaisson ? lPl : 0;
  // yBas = hauteur depuis le bas du latéral où se trouve le centre du panneau inf
  var yBas;
  if (cfg.positionPanneauInf === 'au_dessus_plinthe')      yBas = lPlEff + ep0/2;
  else if (cfg.positionPanneauInf === 'dans_zone_plinthe') yBas = lPlEff - ep0/2;
  else                                                      yBas = ep0/2; // fleur
...
function calculerCharnieres(items) {
  ...
  // Si la plinthe est rapportée (devant le caisson), elle n'affecte pas la
  // position de la porte sur le latéral.
  var hPlEff = cfg.hasPlintheCaisson ? hPl : 0;
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    // - Latéraux porteurs (au_dessus_plinthe ou dans_zone_plinthe) :
    //     porte encastrée : plinthe + panneau inf + jeu
    //     porte applique  : plinthe + jeu (la porte recouvre le panneau inf)
    // - Fleur (nivelement / suspendu / pas de plinthe) :
    //     porte encastrée : panneau inf + jeu
    //     porte applique  : jeu (la porte recouvre le panneau inf à fleur)
    var yPD;
    if (iTP === 'encastree') {
      yPD = hPlEff + iEp + jH;
    } else {
...
function calculerFonds(items) {
  ...
  // Si la plinthe est rapportée (devant le caisson), elle ne mange pas la
  // hauteur intérieure : on neutralise hPl pour le calcul de yBas.
  var hPlEff = cfg.hasPlintheCaisson ? hPl : 0;
  var yBas;
  if (cfg.positionPanneauInf === 'au_dessus_plinthe')      yBas = hPlEff;
  else if (cfg.positionPanneauInf === 'dans_zone_plinthe') yBas = hPlEff - ep0;
  else                                                      yBas = 0; // fleur
