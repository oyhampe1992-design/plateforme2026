...
function genererPlansPDF(nomMeuble) {
  ...
  function dessinerPDF(item, ox, oy, cw, ch) {
    ...
    // Profondeur de reference pour la rainure :
    // 1. _profMeubleRain pose par item (multi-meubles : chaque meuble a sa profondeur)
    // 2. fallback sur _rainures.profMeuble (mono-meuble, retro-compat)
    // 3. fallback ultime sur lP (la pièce elle-même)
    var pMr   = item._profMeubleRain || (window._rainures ? window._rainures.profMeuble : lP);
    ...
    // Pour un latéral : sF='gauche'/'droite' = côté du caisson (miroir)
    // Pour une porte   : sF='gauche'/'droite' = côté de la charnière
    var cL;
    if (type === 'lateral' && (sF==='gauche' || sF==='droite')) {
      cL = sF==='gauche' ? ' — Côté GAUCHE' : ' — Côté DROIT';
    } else {
      cL = sF==='gauche'?' — Charnière GAUCHE':sF==='droite'?' — Charnière DROITE':'';
    }
    ...
    if ((type==='lateral'||type==='montant') && !estPM) {
      ...
      // Détection côté droit : via _sensForce (nouveau dédoublement) OU via designation (legacy)
      var eDroit = (type==='lateral') && (sF==='droite' || nomR.indexOf('droit')>-1);
    ...
    if (type !== 'panneau') {
      ...
      if (type==='porte'||type==='tiroir') { chG=chD=chH=chB=true; }
        ...
        // Détection côté droit : via _sensForce (nouveau dédoublement) OU via designation (legacy)
        var eD2=(type==='lateral')&&(sF==='droite' || nomR.indexOf('droit')>-1);
  ...
  for (var pi = 0; pi < pieces.length; pi++) {
    ...
    } else if (it.type === 'lateral' && it.p.nombre >= 2) {
      // Latéraux : un caisson a 2 latéraux miroir → 1 plan gauche + 1 plan droit
      // (rainure et chant inversés selon le côté)
      var nL = Math.floor(it.p.nombre / 2);
      groups[mK].items.push({ item: it, sensForce: 'gauche', nbAffiche: nL });
      groups[mK].items.push({ item: it, sensForce: 'droite', nbAffiche: nL });
      if (it.p.nombre % 2 !== 0) groups[mK].items.push({ item: it, sensForce: 'gauche', nbAffiche: 1 });
