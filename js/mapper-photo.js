/* ================================================================
   THE WOODER - mapper-photo.js (v9)
   ================================================================
   CHANGELOG v9 :
   - optsFromElement lit ens.pose (mode_support, moyen_nivellement,
     plinthe_rapportee, type_plinthe, hauteur_plinthe) en plus de
     ens.plinthe legacy.
   - Si mode_support !== 'lateraux_porteurs', le typePlinthe legacy
     est forcé à 'aucune' (pas de pièce plinthe entre latéraux —
     la plinthe rapportée éventuelle est gérée séparément en pleine façade).
   - Nouveaux champs dans opts : modeSupport, moyenNivellement,
     plintheRapportee, hauteurPlintheRapportee.
   CHANGELOG v8 :
...
function optsFromElement(el) {
  ...
  // ── Type de plinthe (legacy, lecture depuis ens.plinthe ou se.PLI) ─
  ...
  // ── Nouvelle config pose : si présente, prend le pas sur le legacy ─
  // pose = { mode_support, type_plinthe, moyen_nivellement, plinthe_rapportee, hauteur_plinthe }
  var pose                    = ens.pose || null;
  var modeSupport             = 'lateraux_porteurs';
  var moyenNivellement        = null;
  var plintheRapportee        = false;
  var hauteurPlintheRapportee = 0;

  if (pose) {
    modeSupport             = pose.mode_support || 'lateraux_porteurs';
    moyenNivellement        = pose.moyen_nivellement || null;
    plintheRapportee        = !!pose.plinthe_rapportee;
    hauteurPlintheRapportee = pose.hauteur_plinthe || 0;

    if (modeSupport === 'lateraux_porteurs') {
      // Mode classique : on prend les valeurs de pose si présentes
      if (pose.type_plinthe) typePlinthe = pose.type_plinthe;
      if (typePlinthe === 'encastree' || typePlinthe === 'applique') {
        if (pose.hauteur_plinthe) hPlinthe = pose.hauteur_plinthe;
      }
    } else {
      // Mode nivelement / suspendu : pas de pièce plinthe entre latéraux.
      // On force le typePlinthe legacy à 'aucune' pour que les générateurs
      // existants (qui ne lisent que typePlinthe) ne créent pas de plinthe.
      // La plinthe rapportée éventuelle est portée par plintheRapportee +
      // hauteurPlintheRapportee, à consommer par les générateurs futurs.
      typePlinthe = 'aucune';
    }
  }
  ...
  return {
    ...
    // Nouveaux champs issus de ens.pose
    modeSupport:             modeSupport,
    moyenNivellement:        moyenNivellement,
    plintheRapportee:        plintheRapportee,
    hauteurPlintheRapportee: hauteurPlintheRapportee
  };
}
